...
async def query_chatbot(query_data: QueryRequest):
    ...
    try:
        ...
        prompt = f"""Using the following document content, provide a concise and accurate answer to the question. 
                    If the document contains relevant information, summarize it clearly.
                    If the document does not contain relevant information, state that explicitly. Do not speculate beyond the provided content.
                    Extracted Content:
                    {context}
                    Question:
                    {query_data.query}"""
